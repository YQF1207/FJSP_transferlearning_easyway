...
class Encode:
    def __init__(self,Matrix,Pop_size,J,J_num,M_num, iteration):
        self.Matrix=Matrix      #工件各工序对应各机器加工时间矩阵
        #self.GS_num = int(0 * Pop_size)
        if iteration == 0:
            self.LS_num=int(0.2*Pop_size)     #局部选择初始化0.2   0.1
            self.RS_num=int(0.6*Pop_size)     #随机选择初始化0.2   0.3
            self.GS_num = int(0.2 * Pop_size)  # 全局选择初始化0.6  0.6
        else:
            self.LS_num = int(0.2 * Pop_size)  # 局部选择初始化0.2   0.1
            self.RS_num = int(0.5 * Pop_size)  # 随机选择初始化0.2   0.3
            self.GS_num = int(0.3 * Pop_size)
        self.J=J                #各工件对应的工序数
    ...
    def return_transfer_num(self):
        transfer_num = self.LS_num
        return transfer_num
    ...
    #生成初始化矩阵
    def CHS_Matrix(self, C_num):  # C_num
        return np.zeros([C_num, self.Len_Chromo], dtype=int)  #列数是所有工序和
    ...
    def Global_initial(self):
        ...
        for i in range(self.GS_num):
            ...
            random.shuffle(OS_list)  # 生成工序排序部分  工序随机排列
            OS[i] = np.array(OS_list)
            GJ_list = [i_1 for i_1 in range(self.J_num)]   #工件数量
