...
class Decode2:
    ...
    def Order_Matrix(self,MS):
        ...
        for i in range(len(Ms_decompose)):
            ...
            for j in range(len(Ms_decompose[i])):
                ...
                # print(i,j)
                # print(Ms_decompose)
                # print(Ms_decompose[i][j])
                # print(M_ij)
                # print(M_ij[Ms_decompose[i][j]])
                # JM_i.append(M_ij[Ms_decompose[i][j]])
                # T_i.append(T_ij[Ms_decompose[i][j]])
                # print(Ms_decompose[i][j])
                # print(M_ij)
                # print(len(M_ij))
                if (Ms_decompose[i][j]+1) <= len(M_ij):
                    JM_i.append(M_ij[Ms_decompose[i][j]])
                    T_i.append(T_ij[Ms_decompose[i][j]])
                else:
                    JM_i.append(M_ij[-1])
    ...
    def Gantt(self,Machines, iteration, factory_num,assignment,useless_job,job_a,job_b,num,move):
        M = ['red', 'blue', 'yellow', 'orange', 'green', 'palegoldenrod', 'purple', 'pink', 'Thistle', 'Magenta',
             'SlateBlue', 'RoyalBlue', 'Cyan', 'Aqua', 'floralwhite', 'ghostwhite', 'goldenrod', 'mediumslateblue',
             'navajowhite','navy', 'sandybrown', 'moccasin']
        N = [ '.','--','++','oo','x','||','//','\\','xx','O','*']

        # for u in range(len(useless_job)):
        #     useless_job[u] += 1
        # print('useless__job:', useless_job)
        ...
        for i in range(len(Machines)):
            ...
            for i_1 in range(len(End_time)):
                # print(Machine.assigned_task[i_1])
                # print(Start_time[i_1])
                # print(End_time[i_1])
                # print('...............')

                if End_time[i_1] !=0 and End_time[i_1] - Start_time[i_1]!= 0:
                    plt.barh(i,width=End_time[i_1]-Start_time[i_1],height=0.8,left=Start_time[i_1],\
                          color='white',edgecolor='black',hatch=N[Machine.assigned_task[i_1][0]])
                    plt.text(x=Start_time[i_1]+0.1,y=i,s=assignment[Machine.assigned_task[i_1][0]-1],fontsize=6)
                # else:
                #     #print(Machine.assigned_task[i_1])
                #     plt.barh(i, width=End_time[i_1] - Start_time[i_1], height=0.8, left=Start_time[i_1], \
                #                  color='white', edgecolor='black')
                #     plt.text(x=Start_time[i_1] + (End_time[i_1] - Start_time[i_1])/2-0.5, y=i, s=assignment[Machine.assigned_task[i_1][0]-1],fontsize=6)
        ...
        if move:
            plt.savefig(
                "./gantte_move/"'gant_' + str(iteration) + '_' + str(job_a) + '_' + str(job_b) + '_' + str(num)+"move")
        else:
            plt.savefig("./gantte_exchange/"'gant_'+str(iteration)+'_'+str(job_a)+'_'+str(job_b)+'_'+str(num))
