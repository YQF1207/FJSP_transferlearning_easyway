...
class Decode:
    ...
    def Gantt(self,Machines, iteration, factory_num,assignment,useless_job):
        M = ['red', 'blue', 'yellow', 'orange', 'green', 'palegoldenrod', 'purple', 'pink', 'Thistle', 'Magenta',
             'SlateBlue', 'RoyalBlue', 'Cyan', 'Aqua', 'floralwhite', 'ghostwhite', 'goldenrod', 'mediumslateblue',
             'navajowhite','navy', 'sandybrown', 'moccasin']
        N = [ '.','--','++','oo','x','||','//','\\','xx','O','*']

        # useless = copy.deepcopy(useless_job)
        # for u in range(len(useless)):
        #     useless[u] += 1
        # print('useless__job:', useless)
        ...
        plt.ylabel('Machines')
        plt.xlabel('Time(s)')
        plt.savefig("./gantte/"'gant_'+str(iteration)+'_'+str(factory_num))
